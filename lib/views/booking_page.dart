...
class _BookingPageState extends State<BookingPage> {
  ...
  bool _isBookingOverlap(
    ...
    final existingBookings = _bookingBox.values.where((booking) {
      return booking.namaLapangan == namaLapangan &&
          DateUtils.isSameDay(booking.tanggalBooking, tanggalBooking);
    });
  ...
  void _saveBookingToHive() async {
    ...
    try {
      ...
      await NotificationService().showBookingNotification(
        ...
        body:
            'Booking lapangan $_lapanganNama pada ${DateFormat('dd MMM yyyy').format(_selectedDay)} jam $_selectedTime berhasil dibuat.',
      );

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Booking berhasil disimpan! (Langsung ke Hive)'),
          ),
        );
        ...
        children: [
          Text(
            ...
            style: TextStyle(
              ...
              color: isTotal ? Colors.black : Colors.black.withOpacity(0.5),
            ),
          ),
          Text(
            value,
